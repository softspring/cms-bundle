...
class PublishListener extends AbstractSectionVersionListener
{
    ...
    public function onSuccess(SuccessEvent $event): void
    {
        ...
        $this->flashNotifier->addTrans('success', 'admin_sections.version_publish.success_flash', [], 'sfs_cms_admin');

        $event->setResponse($this->redirectBack($section, $event->getRequest(), $event->getEntity()));
    }

    public function onFailure(FailureEvent $event): void
    {
        $this->flashNotifier->addTrans('error', 'admin_sections.version_publish.failed_flash', ['%exception%' => $event->getException()->getMessage()], 'sfs_cms_admin');
