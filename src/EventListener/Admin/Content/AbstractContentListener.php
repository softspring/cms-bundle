...
abstract class AbstractContentListener implements EventSubscriberInterface
{
    ...
    {
        $eventName = str_replace('sfs_cms.admin.contents.', '', $eventName);

        $contentConfig = $event->getRequest()->attributes->get('_content_config');

        $dispatcher->dispatch($event, sprintf('sfs_cms.admin.contents.%s.%s', $contentConfig['_id'], $eventName));
    }
